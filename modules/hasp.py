...
def execute_command(command, module, result, passw, req):
    p = subprocess.run(['sudo', '-S'] + command.split(), universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=passw.stdout)
...
def run_module():
    ...
    passw = subprocess.Popen(['echo', '0205'], stdout=subprocess.PIPE)
    p = subprocess.run(['sudo', '-S'], universal_newlines=True, stdin=passw.stdout, stdout=subprocess.PIPE)
    ...
    command1 = "wget " + url
    execute_command(command1, module, result, passw, False)
    
    command2 = 'apt update'
    execute_command(command2, module, result, passw, False)
    
    command3 = 'dpkg -i haspd*.deb'
    execute_command(command3, module, result, passw, False)
    
    command4 = 'systemctl restart haspd'
    execute_command(command4, module, result, passw, False)
